...
<font style="color:#FF6600;font-size:19px;">sql调试工具，sql追踪</font>
...
else
{
	...
	{	
		...
		//if(preg_match("/Connect/", $str))
			...
			$str = str_replace('Query', '<span style="color:red">================================================</span><br/>', $str);						
			$str = preg_replace('/([0-9]{1,} Query)|([0-9]{1,} Quit)/', "", $str);
			$str = preg_replace('/([0-9]{1,})\sInit/', "Init", $str);
			if(stripos($str, 'select') || stripos($str, 'update') || stripos($str, 'insert') || stripos($str, 'delete')){
				echo $str = "<p>{$str}</p>";
			}else{
